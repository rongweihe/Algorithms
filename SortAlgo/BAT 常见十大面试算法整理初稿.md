...
void heapAdjust2(int *a,int s,int m){
    ...
    int L = 2*s+1;
    int R = 2*s+2;
    if(L<m && a[L]>a[largest]) largest=L;
    if(R<m && a[R]>a[largest]) largest=R;
    if(largest!=s){
        swap(a[s],a[largest]);
        heapAdjust(a,largest,m);
    }
}
/*非递归版本*/
void heapAdjust(int *a,int s,int m){
    ...
    for(int i=2*s; i<=m; i*=2){
        if(i<m && a[i]<a[i+1]) ++i;//左孩子小于右孩子
        if(tp>=a[i])//i为关键字较大的记录下标
            break;
        a[s]=a[i];
        s=i;
    }
